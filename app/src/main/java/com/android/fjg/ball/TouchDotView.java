...
/**
 * Created by fujianguo on 16-8-22.
 */
public class TouchDotView extends LinearLayout {
    ...
    public TouchDotView(Context context) {
        ...
        imageview.setImageResource(R.drawable.default_sprite_touch_icon_selector);
    ...
    public TouchDotView(Context context, int x, int y, int layoutres) {
        super(context);
        View view = LayoutInflater.from(getContext()).inflate(layoutres, null);
        init(view, x, y);
    }

    public TouchDotView(Context context, int x, int y, View childView) {
        super(context);
        init(childView, x, y);
    }
    ...
    private void init(View childView, int x, int y) {
        ...
        wmParams.x = x;
        wmParams.y = y;
        wmParams.windowAnimations = R.style.RightDotAnimalStyle;
        if (childView != null) {
            addView(childView);
        }
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (Math.abs(y - mTouchStartY) > 10 || Math.abs(x - mTouchStartX) > 10) {
                    if (x > mScreenWidth / 2) {
                        ...
                        wmParams.windowAnimations = R.style.LeftDotAnimalStyle;
                        wmParams.x = 0;
                    }
                    wm.updateViewLayout(this, wmParams);
                } else {
